import { DateOnMenu, MenuOnProduct, Product, ProductOnOrder } from "@/interface/types"; // Assuming you have a Product type defined in types
...
export function updateProductOnOrder(weekdays: DateOnMenu[], products: Product[]): ProductOnOrder[] {
        ...
        day.menuOnMeal.forEach((meal) => {
            if (meal.quantity > 0) {
                ...
                if (product) {
                    ...
                    productMap[meal.sku].sub_price = productMap[meal.sku].quantity * product.unit_price;
                }
            }
        });
    });
...
export function updateMenuOnProduct(weekdays: DateOnMenu[], products: Product[]): MenuOnProduct[] {
